...
export function encodeExternalCall(target: Address, value: bigint, useSelfBalance: boolean, data: Hex): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.EXT_CALL), target, generateAmountBitmap(uint128(value), false, useSelfBalance), uint16(data.length / 2 - 1), data]
    )
}
...
export function encodeAcrossBridgeToken(
    ...
    const bridgeData = encodePacked(
        ["uint8", "uint8", "address", "address", "address", "bytes32", "uint128", "uint128", "uint32", "uint32", "bytes32", "uint16", "bytes"],
        [
...
export function encodeNextGenDexUnlock(singleton: Address, id: bigint, d: Hex): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.GEN_2025_SINGELTONS), uint8(Gen2025ActionIds.UNLOCK), singleton, uint16(d.length / 2 - 1 + 1), uint8(id), d]
    )
}
...
export function encodeBalancerV3Take(singleton: Address, asset: Address, receiver: Address, amount: bigint): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.GEN_2025_SINGELTONS), uint8(Gen2025ActionIds.BAL_V3_TAKE), singleton, asset, receiver, uint128(amount)]
    )
}
...
export function encodeUniswapV4Take(singleton: Address, asset: Address, receiver: Address, amount: bigint): Hex {
    return encodePacked(
        ["uint8", "uint8", "address", "address", "address", "uint128"],
        [uint8(ComposerCommands.GEN_2025_SINGELTONS), uint8(Gen2025ActionIds.UNI_V4_TAKE), singleton, asset, receiver, uint128(amount)]
    )
}
...
export function swapHead(amount: bigint, amountOutMin: bigint, assetIn: Address): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.SWAPS), generateAmountBitmap(uint128(amount), false, false), uint128(amountOutMin), assetIn]
    )
}
...
export function encodeBalancerV2StyleSwap(
    ...
    return encodePacked(
        ...
        [currentData, tokenOut, receiver, uint8(DexTypeMappings.BALANCER_V2_ID), poolId, balancerVault, uint8(uint256(cfg))]
    )
}
...
export function encodeLbStyleSwap(currentData: Hex, tokenOut: Address, receiver: Address, pool: Address, swapForY: boolean, cfg: any): Hex {
    if (cfg === DexPayConfig.FLASH) throw new Error("Invalidconfigforv2swap")
    return encodePacked(
        ["bytes", "address", "address", "uint8", "address", "uint8", "uint8"],
        [currentData, tokenOut, receiver, uint8(DexTypeMappings.LB_ID), pool, uint8(swapForY ? 1 : 0), uint8(uint256(cfg))]
    )
}
...
export function encodeGmxStyleSwap(currentData: Hex, tokenOut: Address, receiver: Address, pool: Address, cfg: any): Hex {
    ...
    return encodePacked(
        ["bytes", "address", "address", "uint8", "address", "uint8"],
        [currentData, tokenOut, receiver, uint8(DexTypeMappings.GMX_ID), pool, uint8(uint256(cfg))]
    )
}
...
export function encodeKtxStyleSwap(currentData: Hex, tokenOut: Address, receiver: Address, pool: Address, cfg: any): Hex {
    ...
    return encodePacked(
        ["bytes", "address", "address", "uint8", "address", "uint8"],
        [currentData, tokenOut, receiver, uint8(DexTypeMappings.KTX_ID), pool, uint8(uint256(cfg))]
    )
}
...
export function encodeCurveNGStyleSwap(
    ...
    return encodePacked(
        ["address", "address", "uint8", "address", "uint8", "uint8", "uint8", "uint16"],
        [tokenOut, receiver, uint8(DexTypeMappings.CURVE_RECEIVED_ID), pool, uint8(indexIn), uint8(indexOut), uint8(selectorId), uint16(uint256(cfg))]
    )
}

export function encodeWrapperSwap(currentData: Hex, assetOut: Address, receiver: Address, operation: any, cfg: any): Hex {
    return encodePacked(
        ["bytes", "address", "address", "uint8", "uint8", "uint8"],
        [currentData, assetOut, receiver, uint8(DexTypeMappings.ASSET_WRAP_ID), uint8(uint256(operation)), uint8(uint256(cfg))]
    )
}
...
export function encodeNextGenDexSettle(singleton: Address, nativeAmount: bigint): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.GEN_2025_SINGELTONS), uint8(Gen2025ActionIds.UNI_V4_SETTLE), singleton, uint128(nativeAmount)]
    )
}
...
export function encodeWrap(amount: bigint, wrapTarget: Address): Hex {
    return encodePacked(
        ["uint8", "uint8", "address", "address", "uint8", "uint128"],
        [uint8(ComposerCommands.TRANSFERS), uint8(TransferIds.SWEEP), zeroAddress, wrapTarget, uint8(SweepType.AMOUNT), uint128(amount)]
    )
}
...
export function encodeUnwrap(target: Address, receiver: Address, amount: bigint, sweepType: any): Hex {
    return encodePacked(
        ["uint8", "uint8", "address", "address", "uint8", "uint128"],
        [uint8(ComposerCommands.TRANSFERS), uint8(TransferIds.UNWRAP_WNATIVE), target, receiver, sweepType, uint128(amount)]
    )
}
...
export function encodeErc4646Withdraw(vault: Address, isShares: boolean, assets: bigint, receiver: Address): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.ERC4626), uint8(1), vault, generateAmountBitmap(uint128(assets), isShares, false), receiver]
    )
}
...
export function encodeMorphoWithdrawCollateral(market: Hex, assets: bigint, receiver: Address, morphoB: Address): Hex {
    return encodePacked(
        ["uint8", "uint8", "uint16", "bytes", "uint128", "address", "address"],
        [uint8(ComposerCommands.LENDING), uint8(LenderOps.WITHDRAW), uint16(LenderIds.UP_TO_MORPHO), market, uint128(assets), receiver, morphoB]
    )
}
...
export function encodeCompoundV3Borrow(token: Address, amount: bigint, receiver: Address, comet: Address): Hex {
    return encodePacked(
        ...
        [uint8(ComposerCommands.LENDING), uint8(LenderOps.BORROW), uint16(LenderIds.UP_TO_COMPOUND_V3 - 1), token, uint128(amount), receiver, comet]
    )
}
...
export function encodeCompoundV2Borrow(token: Address, amount: bigint, receiver: Address, cToken: Address): Hex {
    return encodePacked(
        ["uint8", "uint8", "uint16", "address", "uint128", "address", "address"],
        [uint8(ComposerCommands.LENDING), uint8(LenderOps.BORROW), uint16(LenderIds.UP_TO_COMPOUND_V2 - 1), token, uint128(amount), receiver, cToken]
    )
}
