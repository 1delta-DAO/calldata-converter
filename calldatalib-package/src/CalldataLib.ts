// @ts-nocheck
  ...
  import { encodePacked, uint128, uint8, uint112, uint16, uint256, _PRE_PARAM, _SHARES_MASK, _UNSAFE_AMOUNT, generateAmountBitmap, newbytes, bytes, getMorphoCollateral, getMorphoLoanAsset } from "./utils";
...
return  encodePacked(['uint128', 'uint32', 'uint128', 'uint8', 'uint16', 'uint16', 'bytes', 'bytes'], [generateAmountBitmap(uint128(amount),false,false,isNative), slippage, uint128(fee), uint8(isBusMode?1:0), uint16(composeMsg.length/2 -1), uint16(extraOptions.length/2 -1), composeMsg, extraOptions]);}
...
export function encodeAcrossBridgeToken(spokePool: Address, depositor: Address, sendingAssetId: Address, receivingAssetId: Address, amount: bigint, fixedFee: bigint, feePercentage: number, destinationChainId: number, receiver: Address, message: Hex): Hex {
const bridgeData= encodePacked(['uint8', 'uint8', 'address', 'address', 'address', 'address', 'uint128', 'uint128', 'uint32', 'uint32', 'address', 'uint16', 'bytes'], [uint8(ComposerCommands.BRIDGING), uint8(BridgeIds.ACROSS), spokePool, depositor, sendingAssetId, receivingAssetId, generateAmountBitmap(uint128(amount),false,false,false), fixedFee, feePercentage, destinationChainId, receiver, uint16(message.length/2 -1), message]);return bridgeData;}

export function encodeAcrossBridgeNative(spokePool: Address, depositor: Address, sendingAssetId: Address, receivingAssetId: Address, amount: bigint, fixedFee: bigint, feePercentage: number, destinationChainId: number, receiver: Address, message: Hex): Hex {
const bridgeData= encodePacked(['uint8', 'uint8', 'address', 'address', 'address', 'address', 'uint128', 'uint128', 'uint32', 'uint32', 'address', 'uint16', 'bytes'], [uint8(ComposerCommands.BRIDGING), uint8(BridgeIds.ACROSS), spokePool, depositor, sendingAssetId, receivingAssetId, generateAmountBitmap(uint128(amount),false,false,true), fixedFee, feePercentage, destinationChainId, receiver, uint16(message.length/2 -1), message]);return bridgeData;}
...
return  encodePacked(['uint8', 'uint128', 'uint128', 'address'], [uint8(ComposerCommands.SWAPS), generateAmountBitmap(uint128(amount),false,false,false), uint128(amountOutMin), assetIn]);}
...
if(cfg===DexPayConfig.FLASH)throw new Error("Invalidconfigforv2swap");return  encodePacked(['bytes', 'address', 'address', 'uint8', 'bytes32', 'address', 'uint16'], [currentData, tokenOut, receiver, uint8(DexTypeMappings.BALANCER_V2_ID), poolId, balancerVault, uint16(uint256(cfg))]);}
...
if(cfg===DexPayConfig.FLASH)throw new Error("Invalidconfigforv2swap");return  encodePacked(['bytes', 'address', 'address', 'uint8', 'address', 'uint16'], [currentData, tokenOut, receiver, uint8(DexTypeMappings.SYNC_SWAP_ID), pool, uint16(uint256(cfg))]);}
...
return  encodePacked(['bytes', 'uint8', 'uint8', 'uint16', 'bytes', 'uint128', 'address', 'address', 'uint16', 'bytes'], [encodeApprove(getMorphoLoanAsset(market),morphoB), uint8(ComposerCommands.LENDING), uint8(LenderOps.DEPOSIT_LENDING_TOKEN), uint16(LenderIds.UP_TO_MORPHO), market, generateAmountBitmap(uint128(assets),isShares,false,false), receiver, morphoB, uint16(data.length/2 -1>0?data.length/2 -1+1:0), data.length/2 -1 === 0?newbytes(0):encodeUint8AndBytes(uint8(pId),data)]);}

export function encodeErc4646Deposit(asset: Address, vault: Address, isShares: boolean, assets: bigint, receiver: Address): Hex {
return  encodePacked(['bytes', 'uint8', 'uint8', 'address', 'address', 'uint128', 'address'], [encodeApprove(asset,vault), uint8(ComposerCommands.ERC4626), uint8(0), asset, vault, generateAmountBitmap(uint128(assets),isShares,false,false), receiver]);}

export function encodeErc4646Withdraw(vault: Address, isShares: boolean, assets: bigint, receiver: Address): Hex {
return  encodePacked(['uint8', 'uint8', 'address', 'uint128', 'address'], [uint8(ComposerCommands.ERC4626), uint8(1), vault, generateAmountBitmap(uint128(assets),isShares,false,false), receiver]);}

export function encodeMorphoWithdraw(market: Hex, isShares: boolean, assets: bigint, receiver: Address, morphoB: Address): Hex {
return  encodePacked(['uint8', 'uint8', 'uint16', 'bytes', 'uint128', 'address', 'address'], [uint8(ComposerCommands.LENDING), uint8(LenderOps.WITHDRAW_LENDING_TOKEN), uint16(LenderIds.UP_TO_MORPHO), market, generateAmountBitmap(uint128(assets),isShares,false,false), receiver, morphoB]);}
...
return  encodePacked(['uint8', 'uint8', 'uint16', 'bytes', 'uint128', 'address', 'address'], [uint8(ComposerCommands.LENDING), uint8(LenderOps.BORROW), uint16(LenderIds.UP_TO_MORPHO), market, generateAmountBitmap(uint128(assets),isShares,false,false), receiver, morphoB]);}

export function encodeMorphoRepay(market: Hex, isShares: boolean, unsafe: boolean, assets: bigint, receiver: Address, data: Hex, morphoB: Address, pId: bigint): Hex {
return  encodePacked(['bytes', 'uint8', 'uint8', 'uint16', 'bytes', 'uint128', 'address', 'address', 'uint16', 'bytes'], [encodeApprove(getMorphoLoanAsset(market),morphoB), uint8(ComposerCommands.LENDING), uint8(LenderOps.REPAY), uint16(LenderIds.UP_TO_MORPHO), market, generateAmountBitmap(uint128(assets),isShares,unsafe,false), receiver, morphoB, uint16(data.length/2 -1>0?data.length/2 -1+1:0), data.length/2 -1 === 0?newbytes(0):encodeUint8AndBytes(uint8(pId),data)]);}
