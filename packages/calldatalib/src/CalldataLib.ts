...
export const FLUID_USE_BALANCE: bigint = (1n << 127n) - 1n;

export const FLUID_MAX_AMOUNT: bigint = (1n << 112n) - 1n;

export const FLUID_SMART_USE_BALANCE: bigint = (1n << 255n) - 1n;
...
export function encodeTryExternalCallWithReplace(
	...
	return encodePacked(
		["uint8", "address", "uint128", "address", "uint16", "uint16", "uint8", "uint16", "bytes", "bytes"],
		[
...
export function encodeStargateV2Bridge(
	...
	let partialData: Hex = encodeStargateV2BridgePartial(
		amount,
		slippage,
		fee,
		isBusMode,
	...
	return encodePacked(
		["uint8", "uint8", "address", "address", "uint32", "bytes32", "address", "bytes"],
		[
...
export function encodeStargateV2BridgePartial(
	...
	return encodePacked(
		["uint128", "uint32", "uint128", "uint8", "uint16", "uint16", "bytes", "bytes"],
		[
...
export function encodeSquidRouterCall(
	...
	let partialData: Hex = encodeSquidRouterCallPartial(
		asset,
		gateway,
		bridgedTokenSymbol,
		amount,
...
export function encodeBalancerV3FlashLoan(
	...
	let take: Hex = encodeBalancerV3Take(singleton, asset, receiver, amount);
	let settle: Hex = encodeNextGenDexSettleBalancer(singleton, asset, amount);
...
export function encodeUniswapV4FlashLoan(
	...
	let take: Hex = encodeUniswapV4Take(singleton, asset, receiver, amount);
	let settle: Hex = encodeNextGenDexSettle(singleton, asset === zeroAddress ? amount : 0n);
	let sync: Hex = encodeUniswapV4Sync(singleton, asset);
...
export function encodeSweep(asset: Address, receiver: Address, amount: bigint, sweepType: SweepType): Hex {
	return encodePacked(
...
export function encodeWrapWithReceiver(amount: bigint, weth: Address, receiver: Address): Hex {
	return encodePacked(
		["uint8", "uint8", "address", "address", "uint128"],
		[uint8(ComposerCommands.TRANSFERS), uint8(TransferIds.WRAP), weth, receiver, uint128(amount)],
	);
}
...
export function encodeSweepNft(collection: Address, receiver: Address, tokenId: bigint): Hex {
	return encodePacked(
		["uint8", "uint8", "address", "address", "uint256"],
		[uint8(ComposerCommands.TRANSFERS), uint8(TransferIds.SWEEP_NFT), collection, receiver, tokenId],
	);
}

export function encodeUnwrap(target: Address, receiver: Address, amount: bigint, sweepType: SweepType): Hex {
	return encodePacked(
		["uint8", "uint8", "address", "address", "uint8", "uint128"],
		[
			uint8(ComposerCommands.TRANSFERS),
...
export function encodeListaSupplyCollateralViaProvider(
	...
	return encodePacked(
		["uint8", "uint8", "uint16", "bytes", "uint128", "address", "address", "uint16", "bytes"],
		[
			uint8(ComposerCommands.LENDING),
			uint8(LenderOps.DEPOSIT),
			uint16(LenderIds.UP_TO_MORPHO - 1),
...
export function encodeAaveV4Deposit(
	...
	return encodePacked(
		["bytes", "uint8", "uint8", "uint16", "address", "uint128", "address", "uint256", "address", "address"],
		[
...
export function encodeAaveV4Borrow(
	...
	return encodePacked(
		["uint8", "uint8", "uint16", "address", "uint128", "address", "uint256", "address", "address"],
		[
			uint8(ComposerCommands.LENDING),
			uint8(LenderOps.BORROW),
			uint16(LenderIds.UP_TO_AAVE_V4 - 1),
...
export function encodeAaveV4BorrowPermit(
	...
	let data: Hex = encodePacked(
		["address", "uint256", "uint256", "uint256", "uint32", "bytes32", "bytes32"],
...
export function encodeFluidT1Operate(
	...
	let body: Hex = encodePacked(
		["address", "address", "int128", "int128", "uint256", "address", "address", "address"],
		[colUnderlying, debtUnderlying, colAmount, debtAmount, nftId, receiver, nftReceiver, vault],
	);
	return encodePacked(
...
export function encodeFluidDeposit(
	...
	let colAmount: bigint = amount === 0n ? FLUID_USE_BALANCE : int128(amount);
	return encodeFluidT1Operate(underlying, zeroAddress, colAmount, 0n, nftId, receiver, zeroAddress, vault);
}
...
export function encodeFluidBorrow(
	...
	return encodeFluidT1Operate(zeroAddress, underlying, 0n, int128(amount), nftId, receiver, zeroAddress, vault);
}
...
export function encodeFluidRepay(
	...
	let debtAmount: bigint = amount === 0n || amount === FLUID_MAX_AMOUNT ? FLUID_ALL : -int128(amount);
	return encodeFluidT1Operate(zeroAddress, underlying, 0n, debtAmount, nftId, receiver, zeroAddress, vault);
}
...
export function encodeFluidWithdraw(
	...
	let colAmount: bigint = amount === FLUID_MAX_AMOUNT ? FLUID_ALL : -int128(amount);
	return encodeFluidT1Operate(underlying, zeroAddress, colAmount, 0n, nftId, receiver, zeroAddress, vault);
}
...
export function _fluidSmartHeader(
	...
	return encodePacked(
		["uint8", "uint8", "uint16", "uint8", "uint128", "uint256", "address", "address", "address"],
		[
			uint8(ComposerCommands.LENDING),
...
export function encodeFluidFTokenDeposit(underlying: Address, amount: bigint, receiver: Address, fToken: Address): Hex {
	return encodePacked(
		["bytes", "uint8", "uint8", "uint16", "address", "uint128", "address", "address"],
		[
...
export function encodeGearboxV3RepayPartial(
	...
	if (amount === 0n || amount === GEARBOX_REPAY_ALL) {
		(() => {
			throw new Error("CL: gearbox partial repay needs literal amount");
		})();
	}
	return encodePacked(
		["bytes", "uint8", "uint8", "uint16", "address", "uint128", "address", "uint8"],
		[
...
export function encodeGearboxV3RepayAll(
	...
	if (quotedTokens.length > 255) {
		(() => {
			throw new Error("CL: gearbox too many quoted tokens");
		})();
	}
	let quotedBlob: Hex = "0x" as Hex;
	for (let i: number = 0; i < quotedTokens.length; i++) {
		quotedBlob = encodePacked(["bytes", "address"], [quotedBlob, quotedTokens[i]]);
	}
	return encodePacked(
		["bytes", "uint8", "uint8", "uint16", "address", "uint128", "address", "uint8", "bytes"],
		[
			encodeApprove(underlying, creditManager),
			uint8(ComposerCommands.LENDING),
			uint8(LenderOps.REPAY),
...
export function encodeGearboxV3FacadeCall(innerCallData: Hex): Hex {
	if (innerCallData.length / 2 - 1 > (1n << 16n) - 1n) {
		(() => {
			throw new Error("CL: gearbox sub-call too long");
		})();
	}
	return encodePacked(["uint16", "bytes"], [uint16(innerCallData.length / 2 - 1), innerCallData]);
}

export function encodeGearboxV3BotMulticall(creditAccount: Address, numCalls: number, calls: Hex): Hex {
	return encodePacked(
		["uint8", "uint8", "uint16", "uint8", "address", "address", "bytes32", "uint16", "bytes"],
		[
			uint8(ComposerCommands.LENDING),
...
export function encodeGearboxV3OpenCreditAccount(
	...
	return encodePacked(
		["uint8", "uint8", "uint16", "uint8", "address", "address", "uint256", "uint16", "bytes"],
		[
			uint8(ComposerCommands.LENDING),
			uint8(LenderOps.GEARBOX_MULTICALL),
			uint16(LenderIds.UP_TO_GEARBOX_V3 - 1),
