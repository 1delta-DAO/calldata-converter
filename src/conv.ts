...
import type { SolidityStruct, FunctionDef, ConverterOutput, Parameter } from "./types"
...
function convertType(solidityType: string, structs: SolidityStruct[] = []): string {
    // Handle array types
...
function getFunctionReturnType(functionName: string, functions: FunctionDef[]): string {
 ...
 */
function parseTernaryType(expression: string, functions: FunctionDef[], params: Parameter[]): string {
    // split ternary into parts
    const [condition, truePart, falsePart] = expression.split(/\?|:/).map((p) => p.trim())
    ...
    const getExprType = (expr: string): string => {
        ...
        if (castMatch && (castMatch[0]?.startsWith("uint") || castMatch[0]?.startsWith("address"))) {
            return castMatch[1]!
        }
...
function convertAbiEncodePacked(funcDef: FunctionDef, allFunctions: FunctionDef[]): string {
    ...
    if (funcDef.body.includes("abi.encodePacked")) {
        ...
        if (encodedParams) {
            ...
            args.forEach((arg) => {
                ...
                if (functionCallMatch && arg.indexOf(functionCallMatch?.[0] || "0") === 0) {
                ...
                if (arg.includes("?")) {
                    const inferredType = parseTernaryType(arg, allFunctions, funcDef.params)
                ...
                paramType = paramType.replace(" memory", "").replace(" calldata", "").trim()
            ...
            const cleanedTypes = types.map((type) => (type.startsWith("//") ? type.replace("//", "").trim() : type))

            tsBody = tsBody.replace(/abi\.encodePacked\((.*)\)/, ` encodePacked(['${cleanedTypes.join("', '")}'], [${valueExpressions.join(", ")}])`)
        }
    }
...
function convertLib2Enum(libCode: string): string {
    ...
    const constantRegex = /\w+\s+(?:internal\s+)?(?:constant\s+)?(\w+)\s*=\s*([^;]+);/g
 ...
 */
export function convertToTS(solidityCode: string, debug: boolean = false): ConverterOutput {
    const { functions, enums, constants, structs, imports, libraries } = parseSolidity(solidityCode)

    let output = ""

    output += `
  import { type Hex, type Address, zeroAddress } from "viem";
  import { encodePacked, uint128, uint8, uint112, uint16, uint256, _PRE_PARAM, _SHARES_MASK, _UNSAFE_AMOUNT, generateAmountBitmap, newbytes, bytes, getMorphoCollateral, getMorphoLoanAsset, rightPadZero, encodeCompoundV2SelectorId, encodeSiloV2CollateralMode } from "../../src/utils.ts";
  `
...
function convertFunction(func: FunctionDef, structs: SolidityStruct[], functions: FunctionDef[]): string {
    let output: string = ""
    // Function signature
    output += `export function ${func.name}(`
    output += func.params.map((param) => `${param.name}: ${convertType(param.type, structs)}`).join(", ")
